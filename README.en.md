...
1. Clone este repositório e execute `yarn setup` para gerar os ficheiros `.env`.
2. Personalize os valores dos `.env` em `server` e `frontend` com as suas chaves e configurações.
3. Para desativar a telemetria defina `DISABLE_TELEMETRY=true` conforme indicado no código.
4. Inicie o projeto com `yarn dev:server` e `yarn dev:frontend`.

### Direitos de uso

AnythingLLM é disponibilizado sob a [licença MIT](./LICENSE). Pode utilizar, modificar e redistribuir livremente mantendo este aviso de licença.
